...
fn main() {
    ...
    for coord in arrow_coords.iter() {
        ...
        let dst_from_center = coord.distance_from_center();
        let new_shot = if dst_from_center < 1.0 {
            Shot::Bullseye
        }else if dst_from_center >= 1.0 && dst_from_center <= 5.0 {
            Shot::Hit(dst_from_center)
        }else {
            Shot::Miss
        };
        shots.push(new_shot)
